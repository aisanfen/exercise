<?php
echo 1;//n   每次一调试，i.php就要跑出来就只有这个PHP会跑出来吗，恩恩，我试着把它放在某个文件夹下面，然后PHP运行就要出问题
//。。看起来没毛病。。。。就是啊，但是每次一调试，那个文件就要跑出来 是l.php这个文件会跑出来吗，恩恩我也不知道为什么
...
foreach ($able as $key=>$value) {
	if ($key!=0 && $key%13==0) {
		echo '<br />';//没毛病,在其他地方调试就会出现一个警告，说这个文件不在该项目中。。举个列子
	}
	echo "$value&nbsp;&nbsp;";
}
?></span>
